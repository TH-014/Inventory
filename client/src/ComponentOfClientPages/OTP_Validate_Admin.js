...
import "./otp_validate_admin.css";
import styled from "styled-components";

function  OTP_Validate_Admin(){
	console.log('Inside the otp_validate admin function');
	let callAuth = false;
	let resendCount = 0;
	...
	useEffect(() => {
		if(callAuth)
			return;
		console.log('Inside useEffect of otp of admin.');
	...
	const handleLogin = async (e) =>{
		e.preventDefault();
		try{
			const resFromServer = await axios.post('http://localhost:8000/verifyOTPadmin', {
				otp: otp,
				role: 'admin',
				id: userdetails.id
			});
			...
			if(resFromServer.status === 200 && resFromServer.data !== 'OTP not verified.'){
				if(localStorage.getItem("atoken"))
					localStorage.removeItem("atoken");
				localStorage.setItem("atoken", resFromServer.data);
...
export default OTP_Validate_Admin;
